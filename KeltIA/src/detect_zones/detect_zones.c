...
Zone *find_two_main_zones(int *proj, int size, int *count)
{
    ...
    *count = 0;
    ...
    // Filter very small zones (< 3% of dimension) only if we have more than 2 zones
    if (*count > 2)
    {
    ...
    if (*count > 2)
    {
        ...
        if (*count >= 3)
        {
            ...
            if (third_density < second_density * 0.15)
            {
                *count = 2;
            }
        }
...
ExtractedZones detect_zones(MagickWand *wand)
{
    ...
    if (layout && count >= 2)
    {
        ...
        // FIX 1: If words zone is much narrower than grid (< 60% width), extend it
