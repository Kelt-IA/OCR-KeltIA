...
void solver(char* grid, int height, int width, const char* word, int word_len)
{
    ...
    for (int i = 0; i < height; i++)
    {
        ...
        {
            if (grid[i*width+j] == word[0]) // grid[i][j] => grid[i*width+j]
            {
                for (int d = 0; d < 8; d++)
                {
                    ...
                    if (k == word_len) {
                        ...
                        printf("(%i,%i)(%i,%i)\n", j, i, j2, i2); // Python and C's indexes (i,j) are (j,i) humans coordinates.
                        return;
                    }
                }
            }
